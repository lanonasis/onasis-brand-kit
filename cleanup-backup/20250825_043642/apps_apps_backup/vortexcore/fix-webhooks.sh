...
configure_api_keys() {
    ...
    if [[ "$update_keys" =~ ^[Yy]$ ]]; then
        ...
        if [ -n "$supabase_anon_key" ]; then
            # Update .env file
            if grep -q "VITE_SUPABASE_ANON_KEY=REDACTED_SUPABASE_ANON_KEY
                sed -i '' "s/VITE_SUPABASE_ANON_KEY=REDACTED_SUPABASE_ANON_KEY
            else
        ...
        if [ -n "$openai_api_key" ]; then
            if grep -q "OPENAI_API_KEY=REDACTED_OPENAI_API_KEY
                sed -i '' "s/OPENAI_API_KEY=REDACTED_OPENAI_API_KEY
            else
...
test_webhooks() {
    ...
    for func_dir in supabase/functions/*/; do
        if [ -d "$func_dir" ]; then
            ...
            if curl -s -f "$base_url/$func_name" -H "Authorization: Bearer ${VITE_SUPABASE_ANON_KEY=REDACTED_SUPABASE_ANON_KEY
