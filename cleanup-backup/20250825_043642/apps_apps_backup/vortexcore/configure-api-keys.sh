...
validate_supabase_config() {
    ...
    local supabase_url=$(grep "VITE_SUPABASE_URL=https://<project-ref>.supabase.co
    local supabase_key=$(grep "VITE_SUPABASE_ANON_KEY=REDACTED_SUPABASE_ANON_KEY
    
    check_api_key "Supabase URL" "VITE_SUPABASE_URL=https://<project-ref>.supabase.co
    check_api_key "Supabase Anon Key" "VITE_SUPABASE_ANON_KEY=REDACTED_SUPABASE_ANON_KEY
}
...
validate_ai_config() {
    ...
    local openai_key=$(grep "OPENAI_API_KEY=REDACTED_OPENAI_API_KEY
    local gemini_key=$(grep "GEMINI_API_KEY" .env 2>/dev/null | cut -d'=' -f2)
    
    check_api_key "OpenAI" "OPENAI_API_KEY=REDACTED_OPENAI_API_KEY
...
validate_payment_config() {
    ...
    local stripe_webhook=$(grep "STRIPE_WEBHOOK_SECRET=REDACTED_WEBHOOK_SECRET
    
    if [[ -n "$stripe_secret" ]]; then
        check_api_key "Stripe Secret Key" "STRIPE_SECRET_KEY" "$stripe_secret"
        check_api_key "Stripe Webhook Secret" "STRIPE_WEBHOOK_SECRET=REDACTED_WEBHOOK_SECRET
    else
...
deploy_edge_functions() {
    ...
    if [[ -n "$openai_key" && "$openai_key" != *"your_"* ]]; then
        ...
        supabase secrets set OPENAI_API_KEY=REDACTED_OPENAI_API_KEY
    fi
...
generate_config_guide() {
    ...
    echo "   - Update VITE_SUPABASE_URL=https://<project-ref>.supabase.co
    echo ""
    echo -e "${YELLOW}2. OpenAI API Key:${NC}"
    echo "   - Get your API key from: https://platform.openai.com/api-keys"
    echo "   - Update OPENAI_API_KEY=REDACTED_OPENAI_API_KEY
    echo ""
    ...
    echo "   - Update STRIPE_SECRET_KEY and STRIPE_WEBHOOK_SECRET=REDACTED_WEBHOOK_SECRET
    echo ""
