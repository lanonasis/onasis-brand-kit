...
check_api_keys() {
    ...
    if [[ -z "$OPENAI_API_KEY=REDACTED_OPENAI_API_KEY
        warn "OPENAI_API_KEY=REDACTED_OPENAI_API_KEY
        if [[ -f ".env" ]]; then
            source .env
        fi
        
        if [[ -z "$OPENAI_API_KEY=REDACTED_OPENAI_API_KEY
            echo "❌ No API key found. Please set OPENAI_API_KEY=REDACTED_OPENAI_API_KEY
            echo "   export OPENAI_API_KEY=REDACTED_OPENAI_API_KEY
